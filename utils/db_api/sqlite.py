...
"""
    ...
    def select_all_maxsulot(self):
        sql = """
        SELECT name FROM myfiles_maxsulot
        """
        return self.execute(sql, fetchall=True)
    ...
    def select_menu(self, **kwargs):
        # SQL_EXAMPLE = "SELECT * FROM Users where id=1 AND Name='John'"
        sql = "SELECT id FROM myfiles_menu WHERE "
        sql, parameters = self.format_args(sql, kwargs)

        return self.execute(sql, parameters=parameters, fetchone=True)

    def select_maxsulots(self, **kwargs):
        # SQL_EXAMPLE = "SELECT * FROM Users where id=1 AND Name='John'"
        sql = "SELECT * FROM myfiles_maxsulot WHERE "
        sql, parameters = self.format_args(sql, kwargs)
    ...
    def add_users(self,user_id:int, full_name: str):
        # SQL_EXAMPLE ="INSERT INTO myfiles_teacher(id,name,email)VALUES(1,"john","john@hgmail.com")"

        sql="""
        INSERT INTO myfiles_message (user_id, full_name) VALUES(?,?)
        """
